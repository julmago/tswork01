...
$entry_id = (int)get('id');
$entry = null;
if ($cashbox && $entry_id > 0) {
  $st = db()->prepare("SELECT id, detail, amount, created_at, user_id
    FROM cash_movements
    WHERE id = ? AND cashbox_id = ? AND type = 'entry'
    LIMIT 1");
  $st->execute([$entry_id, (int)$cashbox['id']]);
  $entry = $st->fetch();
}
...
if (is_post() && post('action') === 'save_entry') {
  if (!$cashbox) {
    ...
    if ($created_ts === false) {
      ...
      if (!$user_check->fetchColumn()) {
        ...
        $update = db()->prepare("UPDATE cash_movements
          ...
          WHERE id = ? AND cashbox_id = ? AND type = 'entry'");
        $update->execute([$detail, $amount, $created_at, $user_id, $entry_id, (int)$cashbox['id']]);
...
$detail_value = (string)($entry['detail'] ?? '');
$amount_value = isset($entry['amount']) ? (float)$entry['amount'] : 0.0;
$created_value = (string)($entry['created_at'] ?? '');
$created_input = $created_value !== '' ? date('Y-m-d\TH:i', strtotime($created_value)) : '';
$user_value = (int)($entry['user_id'] ?? 0);
?>
<!doctype html>
<html>
<head>
...
<main class="page">
  <div class="container">
    <div class="page-header">
      <h2 class="page-title">Modificar entrada</h2>
    ...
    <?php elseif (!$entry): ?>
      <div class="alert alert-warning">Entrada no encontrada.</div>
    <?php else: ?>
      <div class="card" style="max-width: 640px;">
        ...
        <form method="post" class="stack">
          ...
          <div class="form-actions">
            ...
            <a class="btn btn-ghost" href="<?= url_path('cash_entries_list.php') ?>">Volver</a>
          </div>
        </form>
      </div>
    <?php endif; ?>
